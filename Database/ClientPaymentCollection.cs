...
public class ClientPaymentCollection : BaseCollection<ClientPayment>
{
    ...
    {
        MySqlCommand cmd = new("DELETE FROM client_payments WHERE client_payment_id = @id");
    ...
    {
        MySqlCommand cmd = new("INSERT INTO client_payments (client_id, bank_id, valor) VALUES (@client_id, @bank_id, @valor)");
        cmd.Parameters.AddWithValue("@client_id", item.ClientId);
        cmd.Parameters.AddWithValue("@bank_id", item.BankId);
        cmd.Parameters.AddWithValue("@valor", item.Amount);
        return cmd;
    }
    ...
    {
        MySqlCommand cmd = new("UPDATE client_payments SET client_id = @client_id, bank_id = @bank_id, valor = @valor WHERE client_payment_id = @id");
        cmd.Parameters.AddWithValue("@id", item.Id);
        cmd.Parameters.AddWithValue("@client_id", item.ClientId);
        cmd.Parameters.AddWithValue("@bank_id", item.BankId);
        cmd.Parameters.AddWithValue("@valor", item.Amount);
